...
- **Inline scripts**: Optional top-level `scripts` in collection `settings.yml`, non-root `folder.yml`, and request YAML accepts string-valued `pre` and/or `post`; empty strings are no-ops, but empty mappings, unknown keys, non-string sources, and standalone external-path-looking sources are rejected. Never resolve or read script files. Canonical YAML preserves literal source whitespace in pre/post order. Each invocation gets a fresh QuickJS runtime and context with the frozen, null-prototype `noodle` namespace exposing `noodle.request`, `noodle.env`, `noodle.run`, `noodle.crypto`, `noodle.random`, and `noodle.time`, plus captured `console`. JavaScript built-ins remain global; top-level await and Promises use controlled VM jobs. Pre/post network calls use only `noodle.runRequest` and `noodle.sendRequest`; bare reader aliases, imports, host APIs, raw fetch, timers, and background work are unavailable. The shared lifecycle is folder merge → environment/RunScope → one substitution pass → pre → HTTP → captures → post → assertions → tests. Script source is not substituted. Use captures for declarative extraction and post for conditional processing. External files, modules, script editors/autocomplete, a Console panel, and importer conversion remain deferred.
...
- **Inherited scripts and tests**: `src/scriptInheritance.ts` collects blocks for the shared lifecycle. Each phase runs collection settings, ancestor folders from outermost to innermost, then the request. Root `folder.yml` is ignored. Empty blocks are no-ops and do not disable inherited blocks. Every block uses a fresh QuickJS invocation; JavaScript locals are isolated, and successful RunScope writes reach later blocks. A pre failure stops remaining pre blocks and HTTP. A post failure rolls back only that block and continues later posts, assertions, and tests. Successful persistence intents retain execution order; runners continue to suppress persistence. Saved `noodle.runRequest()` calls use the same inheritance. Inherited diagnostics include optional `source: { scope, path }` with collection-relative paths; test `errors` retains every block error and legacy `error` remains the first error. CLI, Results, and history retain bounded, redacted origins and diagnostics without source code.
- **Scripted tests**: Optional `tests` in collection `settings.yml`, non-root `folder.yml`, and request YAML accepts an inline string evaluated after declarative assertions in manual sends, `request run`, and collection runners. It uses the existing QuickJS sandbox and limits, with synchronous or async `test(name, callback)` and `expect(value)` matchers, including `.not`, `toMatchSchema`, `toHaveProperty`, `toHaveLength`, `toBeTypeOf`, and `toMatchObject`. Returned Promises and thenables are awaited; tests cannot call network APIs. JSON Schema draft-07 compilation and validation run inside QuickJS with local references and standard formats, without coercion, defaults, or remote references. Tests read the final prepared request, response, environment, RunScope, and applicable cookies through `noodle.*`; request, RunScope, and cookie mutations are rejected, response JSON is frozen, and tests cannot persist values. Tests run after completed HTTP, capture, post, or assertion failures; pre and transport failures leave them unevaluated. Empty source is a successful no-op. Failed callbacks preserve declaration order and allow later tests to continue; top-level errors stop only their block while retaining completed results and allowing later blocks to run. Results expose bounded, redacted `tests: { evaluated, results, logs, error?, errors? }` diagnostics in CLI output, TUI Results, and manual timeline history, excluding test source and runtime values from history. Failed tests or a test script error add the `test` failure category and make automation exit nonzero.
