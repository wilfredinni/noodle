...
- **Inline scripts**: Optional top-level `scripts` accepts string-valued `pre` and/or `post`; empty strings are no-ops, but empty mappings, unknown keys, non-string sources, and standalone external-path-looking sources are rejected. Never resolve or read script files. Canonical YAML preserves literal source whitespace in pre/post order. Each synchronous invocation gets a fresh QuickJS runtime and context with the frozen, null-prototype `noodle` namespace exposing `noodle.request`, `noodle.env`, `noodle.run`, `noodle.crypto`, `noodle.random`, and `noodle.time`, plus captured `console`. JavaScript built-ins remain global; bare Noodle API aliases, imports, host APIs, network access, timers, Promises, and queued work are unavailable. The shared lifecycle is folder merge → environment/RunScope → one substitution pass → pre → HTTP → captures → post → assertions → tests. Script source is not substituted. Use captures for declarative extraction and post for conditional processing. Inherited scripts/tests, external files, async APIs, modules, script editors/autocomplete, a Console panel, and importer conversion remain deferred.
- **Scripted tests**: Optional request-level `tests` accepts an inline string evaluated after declarative assertions in manual sends, `request run`, and collection runners. It uses the existing QuickJS sandbox and limits, with synchronous `test(name, callback)` and `expect(value)` matchers, including `.not`. Tests read the final prepared request, response, environment, RunScope, and applicable cookies through `noodle.*`; request, RunScope, and cookie mutations are rejected, response JSON is frozen, and tests cannot persist values. Tests run after completed HTTP, capture, post, or assertion failures; pre and transport failures leave them unevaluated. Empty source is a successful no-op. Failed callbacks preserve declaration order and allow later tests to continue; top-level errors stop the script while retaining completed results. Results expose bounded, redacted `tests: { evaluated, results, logs, error? }` diagnostics in CLI output, TUI Results, and manual timeline history, excluding test source and runtime values from history. Failed tests or a test script error add the `test` failure category and make automation exit nonzero.
