...
noodle collection <create|list|inspect|audit|run> ... [--json]
...
- **Automation commands**: `workspace list`; `collection create`, `list`, `inspect`, `audit [--fix]`, `run [--env]`; `request create --url --method --collection`, `run [--env]`; and `environment set`. They are non-interactive and support `--json`, which writes one `{ status, data, errors }` envelope and uses a nonzero exit status for invalid input or failed runs.
