...
- **Run-scoped variables**: Every manual TUI send and `request run` gets an isolated `RunScope`; every `collection run` and TUI Runner call shares one scope across its ordered requests. Resolved environment and secret values load first, then successful captures and script writes override the same names, with the latest successful write winning. Failed captures and failed script invocations leave earlier committed values unchanged. Captures are visible to the same request's post script and later requests. Transient values disappear when the call returns. Manual sends and `request run` may persist captures declared with `persist`; collection runs and the TUI Runner ignore persistence. Script writes are transient by default. Manual sends and `request run` honor explicit `noodle.run.set/unset` options `{ persist: "environment" | "secret" }`; collection runs and the TUI Runner suppress persistence.
- **Inline scripts**: Optional top-level `scripts` in collection `settings.yml`, non-root `folder.yml`, and request YAML accepts string-valued `pre` and/or `post`; empty strings are no-ops, but empty mappings, unknown keys, non-string sources, and standalone external-path-looking sources are rejected. Never resolve or read script files. Canonical YAML preserves literal source whitespace in pre/post order. Each synchronous invocation gets a fresh QuickJS runtime and context with the frozen, null-prototype `noodle` namespace exposing `noodle.request`, `noodle.env`, `noodle.run`, `noodle.crypto`, `noodle.random`, and `noodle.time`, plus captured `console`. JavaScript built-ins remain global; bare Noodle API aliases, imports, host APIs, network access, timers, Promises, and queued work are unavailable. The shared lifecycle is folder merge → environment/RunScope → one substitution pass → pre → HTTP → captures → post → assertions → tests. Script source is not substituted. Use captures for declarative extraction and post for conditional processing. External files, async APIs, modules, script editors/autocomplete, a Console panel, and importer conversion remain deferred.
...
├── preRequestScript.ts # Phase-aware pre/post sandbox; thin pre entry point
...
- **Script results**: Requests with either phase return optional `scripts: { evaluated, results }` before capture/assertion result groups. Retain executed results in pre/post order; never invent outcomes for phases that did not run. The existing Results surface uses Pre-request/Post-response labels and shows status, duration, log count, normalized errors with source locations, and expandable redacted logs. Human automation output distinguishes Pre-script/Post-script without printing log text; JSON retains redacted phase-tagged logs/errors. Script failure affects request success and the fixed category order: configuration, execution, script, transport, http, capture, assertion, test. Collection continuation/fail-fast is decided after all available response diagnostics finish. Each invocation retains a 500 ms deadline, 32 MiB runtime memory, 512 KiB stack, 256 KiB source and ordinary bridged/RunScope values, depth 32 bridged JSON, 100 console entries, 64 KiB console text, 4 KiB random bytes, depth 4 console serialization, and fixed 64 MiB WASM memory. Only lazy response text gets the separate 5 MiB UTF-8 limit.
