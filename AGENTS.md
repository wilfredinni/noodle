...
`src/app/cli.ts` — citty main command, delegates to `commands/default.ts` (TUI mode) or `commands/import.ts`. TUI bootstrap extracted to `main.tsx` (`bootstrap()` function).
