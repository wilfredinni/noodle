...
- **Timeline security:** Request snapshots resolve ordinary variables, redact declared environment secrets and settings secrets, and mask sensitive headers, auth credentials, OAuth signatures, assertions, and tokens before persistence. Capture declarations persist in request YAML but are excluded from timeline snapshots; capture results and RunScope values are not persisted, and automation runs do not create timeline entries. Server response fields are stored intact. Timeline YAML and gzip sidecars may still contain sensitive payloads.
