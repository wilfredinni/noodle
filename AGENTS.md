...
bun test                              # all tests (3263 across 208 files)
...
- **Pre-request scripts**: Optional top-level `scripts.pre` source runs after folder merging and one substitution pass, but before HTTP, through the shared request lifecycle. Each synchronous invocation gets a fresh QuickJS runtime and context exposing only `request`, `env`, `run`, `crypto`, and captured `console`; imports, host APIs, network access, timers, Promises, and queued work are unavailable. Request and RunScope mutations are staged and commit only on complete success. Failures skip HTTP and return the `script` failure category with redacted results. Script source and logs never enter timeline history, but successful prepared request mutations do. Treat scripted collections as trusted code because `env.get` can read a selected-environment secret and place it in the following request.
...
├── preRequestScript.ts # Sandboxed synchronous scripts.pre runtime
├── requestLifecycle.ts # Shared merge → substitute → script → send → capture → assert flow
...
- **Pre-request script results**: Requests with `scripts.pre` return an optional `scripts` result group before captures and assertions. The existing Results surface shows status, duration, log count, normalized errors, and expandable redacted logs; human automation output never prints log text. The fixed limits are a 500 ms deadline, 32 MiB runtime memory, 512 KiB stack, 256 KiB source and bridged value, depth 32 bridged JSON, 100 console entries, 64 KiB console text, 4 KiB random bytes, depth 4 console serialization, and fixed 64 MiB WASM memory.
