...
├── requestLifecycle.ts # Shared merge → scope → substitute → pre → HTTP → capture → post → assert
...
- **Script results**: Requests with either phase return optional `scripts: { evaluated, results }` before capture/assertion result groups. Retain executed results in pre/post order; never invent outcomes for phases that did not run. The existing Results surface uses Pre-request/Post-response labels and shows status, duration, log count, normalized errors with source locations, and expandable redacted logs. Human automation output distinguishes Pre-script/Post-script without printing log text; JSON retains redacted phase-tagged logs/errors. Script failure affects request success and the fixed category order: configuration, execution, script, transport, http, capture, assertion. Collection continuation/fail-fast is decided after all available response diagnostics finish. Each invocation retains a 500 ms deadline, 32 MiB runtime memory, 512 KiB stack, 256 KiB source and ordinary bridged/RunScope values, depth 32 bridged JSON, 100 console entries, 64 KiB console text, 4 KiB random bytes, depth 4 console serialization, and fixed 64 MiB WASM memory. Only lazy response text gets the separate 5 MiB UTF-8 limit.
