...
- Every request result has `failureCategories` in fixed order. Categories are `configuration`, `execution`, `transport`, `http`, `capture`, and `assertion`; response-based `http`, `capture`, and `assertion` failures may coexist. Human output labels these as configuration error, request error, transport error, HTTP error, capture failure, and assertion failure. Collection results include `failed`, optional configuration `failure`, and `summary` counts for selection, execution, skips, request outcomes, assertions, captures, duration, and unique failure categories.
...
- The shared request flow is fixed: resolve environment and RunScope; execute the request; construct response views; evaluate captures; commit successful captures; evaluate assertions; then return structured results and, for manual TUI sends only, persist safe history. HTTP error responses still reach capture and assertion evaluation; transport failures do not.
