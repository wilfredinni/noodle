...
- Every request result has `failureCategories` in fixed order. Categories are `configuration`, `execution`, `script`, `transport`, `http`, `capture`, and `assertion`; response-based `http`, `capture`, and `assertion` failures may coexist. Human output labels these as configuration error, request error, script failure, transport error, HTTP error, capture failure, and assertion failure. Collection results include `failed`, optional configuration `failure`, and `summary` counts for selection, execution, skips, request outcomes, assertions, captures, duration, and unique failure categories. There are no aggregate script counters.
...
- Requests with `scripts.pre` return a `scripts` group. Read it from
  ...
  scripts omit the group. A script failure skips HTTP, marks later result groups
  unevaluated, and participates in fail-fast.
...
- The shared request flow is fixed: merge folder overrides; overlay RunScope for substitution; substitute once; execute a staged synchronous pre-script; commit successful request and RunScope mutations; send the prepared request; evaluate and commit captures; then evaluate assertions. HTTP error responses still reach capture and assertion evaluation; transport failures do not. Script RunScope changes commit before HTTP and remain available to later collection requests after later failures. Script request changes remain in memory and are never written back to YAML.
