...
- Optional request-level `tests` runs after declarative assertions whenever a response exists. Use global `test(name, callback)` and `expect(actual)` with read-only `noodle.*` readers. Tests are synchronous; mutations, external files, inherited tests, modules, and async work are unsupported. See [the schema and matcher reference](../schema.md#inline-scripted-tests).
