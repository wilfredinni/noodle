...
**Update mode** (`src/app/commands/update.ts`): Standalone release binaries read the Noodle update manifest and cache its validated version and checksums in `~/.config/noodle/update-cache.json`. A valid cached manifest avoids repeat checks for one hour and remains available for update fallback for seven days. Downloaded binaries must match the manifest SHA-256. Unix replaces the executable atomically. Windows stages the verified `.exe` and an embedded PowerShell helper, returns `restart_required`, then replaces the executable only after the current PID exits. Homebrew installs run `brew upgrade noodle`; Bun development runtimes and Linux musl cannot self-update. When a managed `noodle-use` installation already exists, `updateInstall.ts` or the Windows completion helper refreshes it with the updated executable and reports a retry without rolling back a successful Noodle update if refresh fails.
