...
In split-footer mode with `externalOutputMode: "capture-stdout"`, use `writeToScrollback()` for a one-shot
`ScrollbackSnapshot`. Its writer receives a `ScrollbackRenderContext` and returns the root renderable to commit above
the footer.

For incremental output, `createScrollbackSurface()` returns a `ScrollbackSurface`. Add renderables to `surface.root`,
call `render()` or `settle()`, commit a rendered range with `commitRows()`, and call `destroy()` when finished.
