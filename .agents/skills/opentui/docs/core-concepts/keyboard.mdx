...
Core maps configured binding names to canonical names. It maps `enter` to `return` and `esc` to `escape`. It also maps keypad names to their main-keyboard equivalents.
