...
Yoga alignment positions child boxes. It does not align characters inside a text renderable. Use [`textAlign`](/docs/components/text#align-lines) for left, center, or right alignment of each rendered line inside the text width.
...
`textAlign` does not change measured size. It is a draw offset inside the computed width.
